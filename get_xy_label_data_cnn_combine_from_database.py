...
for i in range(len(gene_pair_index)-1):   #### many sperations
    ...
    for gene_pair in gene_pair_label_array[start_index:end_index]: ## each speration
        ...
        if not sys.argv[1] == 'None':
            ...
            x_tf_bulk = log10(rpkm_bulk[h_gene_list_bulk[x_gene_name]] + 10 ** -2)  ## 249 means the number of samples, users can just remove '[0:249]'
            x_gene_bulk = log10(rpkm_bulk[h_gene_list_bulk[y_gene_name]] + 10 ** -2)
